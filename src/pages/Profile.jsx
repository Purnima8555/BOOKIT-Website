...
const Profile = () => {
  ...
  const [currentlyReading, setCurrentlyReading] = useState({ count: 0, books: [] });
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          contact_no: userResponse.data.contact_no || '',
          address: userResponse.data.address || '',
        });
        setImagePreview(userResponse.data.image ? `http://localhost:3000/profilePicture/${userResponse.data.image}` : "/api/placeholder/150/150");
        ...
        setCurrentlyReading(readingResponse.data);
    ...
    switch (activeTab) {
                  ...
                  recentOrders.map(order => (
                    <div key={order.id} className="border border-gray-300 rounded-lg p-4">
                      ...
                      <div className="text-sm text-gray-600">
                        ...
                        <p>Items: {order.items.map(item => item.book_id.title).join(", ")}</p>
      ...
      case 'currentlyReading':
        return (
          <div className="bg-white rounded-lg shadow-sm border border-gray-300">
            <div className="p-6">
              <h3 className="font-semibold mb-4">Currently Reading</h3>
              <div className="space-y-4 max-h-[400px] overflow-y-auto">
                {currentlyReading.books.length === 0 ? (
                  <p className="text-gray-600">No books currently being read.</p>
                ) : (
                  currentlyReading.books.map(book => (
                    <div key={book.book_id} className="border border-gray-300 rounded-lg p-4 flex items-center">
                      <div className="mr-4">
                        {book.image ? (
                          <img
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">
                          ...
                          <p>Rental Price: Rs {book.rental_price}</p>
                        </div>
                      </div>
                    </div>
                  ))
